...
/// Parse the string `s` as a coordinate pair, like `"400x600"` or `"1.0x0.5"`.
///
/// Specifically, `s` should have the form <left><sep><right>, where <sep> is the character given
/// by the `separator` argument and `left` and `right` are both strings that can be parsed by
...
/// Given the row and the column of a pixel in the output image, return the corresponding point
/// on the complex plane.
///
/// - `bounds` is a pair giving the width and height of the image in pixels
/// - `pixel` is a (column, row) pair indicating a particular pixel in that image
/// - The `upper_left` and `upper_right` parameters are points in the complex plane designating the
...
/// complex plane corresponding to upper-left and lower-right corners of the pixel buffer.
fn render(
    pixels: &mut [u8],
    bounds: (usize, usize),
    upper_left: Complex<f64>,
...
fn test_parse_complex() {
    ...
    assert_eq!(parse_complex(",22"), None);
}
